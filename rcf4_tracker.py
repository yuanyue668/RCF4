...
resnet_params = torch.load('resnet34-333f7ec4.pth')
    ...
    for i in range(response.shape[0]):
        for j in range(response.shape[1]):
            ...
            if pad_response[i+1][j+1]>pad_response[i][j] and pad_response[i+1][j+1]>pad_response[i][j+1]:
                #print("pass 1")
                if pad_response[i+1][j+1]>pad_response[i][j+2] and pad_response[i+1][j+1]>pad_response[i+1][j]:
                    #print("pass 2")
                    if pad_response[i+1][j+1]>pad_response[i+2][j] and pad_response[i+1][j+1]>pad_response[i+2][j+1]:
                        #print("pass 3")
                        if pad_response[i+1][j+1]>pad_response[i+2][j+2] and pad_response[i+1][j+1]>pad_response[i+1][j+2]:
...
def get_filter(resnet_feature_ensemble,yf,cos_window):
    ...
    for i in range(resnet_numlayers):
        ...
        x = ndimage.zoom(resnet_feature, (float(cos_window.shape[0])/resnet_feature.shape[0], float(cos_window.shape[1])/resnet_feature.shape[1], 1),order=1)
...
def tracking(image,search_pos,target_position,window_size,resnet_num,resnet_den,cos_window,scalefactor,update_flag):
    ...
    for i in range(resnet_numlayers):
        ...
        feature = resnet_feature_ensemble[i].data[0].cpu().numpy()
        feature = feature[:, :, 0:31]
        x = ndimage.zoom(feature, (float(cos_window.shape[0]) / feature.shape[0], float(cos_window.shape[1]) / feature.shape[1], 1), order=1)
        x = np.multiply(x, cos_window[:, :, None])
        xf = np.fft.fft2(x, axes=(0, 1))
        response = np.real(np.fft.ifft2(np.divide(np.sum(np.multiply(resnet_num[i], xf), axis=2), (resnet_den[i] + lam)))) * resnet_layerweights[i]
    ...
    h_delta,w_delta = [(center_h - final_response.shape[0]/2)*search_factor*cell_size, (center_w - final_response.shape[1]/2)*search_factor*cell_size]
...
def update_position_filter(image,target_position,window_size,scale_factor,position_yf,position_cos_window,resnet_num,resnet_den,update_rate):
...
class tracker:
    ...
    def track(self, image):
            ...
            self.pos,self.update_flag= tracking(image,self.pos,self.pos,self.sz,self.resnet_num,self.resnet_den,self.cos_window,self.current_scale_factor,self.update_flag)
            ...
            if self.update_flag == True:
                self.resnet_num, self.resnet_den = update_position_filter(image,self.pos,self.sz,self.current_scale_factor,self.yf,self.cos_window,
                                                           self.resnet_num, self.resnet_den,self.interp_factor)
...
if __name__ == '__main__':
    ...
    seq_name = out_param[1]
    seq_path = out_param[2]

    start_frame = int(out_param[3])
    end_frame = int(out_param[4])
    ...
    image_file = os.listdir(seq_path)
    image_file.sort()
    image_file = image_file[start_frame - 1:end_frame]

    initial_frame = cv2.imread(os.path.join(seq_path, image_file[0]))

    position = [float(start_x), float(start_y)]

    size = [float(start_w), float(start_h)]

    tracker = tracker(initial_frame, size, position)

    rf = file(seq_name + '_ST.txt', 'w+')

    # Tracking
    for index in range(len(image_file)):
        tracking_image = cv2.imread(os.path.join(seq_path, image_file[index]))
